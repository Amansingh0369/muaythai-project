...
once every field in `required_for_completion` is answered. `completed_at` is set
...
`GET /api/fighter-cards/` returns the compact card — identity, camp, headline
levels, `has_injury`, `train_around_limitations`, `is_complete` — without the
private free text, which travels only on the detail response.
